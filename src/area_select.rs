...
エリア選択モードの開始・終了、ドラッグ処理、オーバーレイ表示制御を管理し、
直感的な領域選択UXを実現する。

【主要機能】
1. エリア選択モード制御（start/cancel_area_select_mode）
2. リアルタイムオーバーレイ表示（透明矩形 + マウス位置表示）
...
[エリア選択ボタン] → start_area_select_mode()
    ...
    [ドラッグ終了] → 領域確定
    ↓
    [ESCキーまたは完了] → cancel_area_select_mode() → 全オーバーレイ削除 + 状態リセット

【オーバーレイ統合】
- cursor_overlay.rs: 赤丸マウス位置表示（20px円形）
- overlay.rs: 半透明矩形領域表示（選択範囲可視化）
- 双方向連携: マウス移動で両オーバーレイが同期更新
 ...
 *
 * 【視覚フィードバック戦略】
 * 2. 半透明黒オーバーレイ：選択モード中であることを全画面で明示
 * 3. リアルタイム追従：マウス移動に完全同期した視覚効果
 * 
 * 【システム統合】
 ...
 * 
 * 【処理フロー】
 * 重複チェック → マウス位置取得 → 状態更新 → フック開始
 *      ↓
 * 視覚オーバーレイ作成 → ユーザー操作待機状態へ遷移
 * 
 * 【エラーハンドリング】
 * - 重複起動時：警告出力して早期リターン
 ...
 * 2. 視覚オーバーレイ：半透明背景 + 赤色カーソルの完全削除
 * 3. システムフック：キーボードフック監視の停止
 * 4. メモリリソース：GDI オブジェクトの適切な解放
 * 
 * 【安全性設計】
 ...
 * 
 * 【処理順序の重要性】
 * 1. AppState フラグ更新（他モジュールでの状態参照を防止）
 * 2. 視覚効果削除（ユーザーへの即座のフィードバック）
 * 3. システムフック停止（システムリソース解放）
 * 
 * 【呼び出し元パターン】
